...
<!--            <div class="print">
        ...
        <div class="content ">
            <div class="you">
                ...
                <?php echo $form->dropDownListRow($model, 'type', array('transfer' => 'Transfer', 'waste' => 'Waste', 'spoil' => 'Spoil'), array('class' => 'span3', 'empty' => t('choose', 'global'))); ?>
                ...
                <tbody>
                    <tr>
                        <td>
                            <?php
                            echo CHtml::ajaxLink(
                                    $text = '<i class="icon-plus-sign"></i>', $url = url('out/addRow'), $ajaxOptions = array(
                                'type' => 'POST',
                            ...
                            $this->widget('bootstrap.widgets.TbSelect2', array(
                                ...
                                'events' => array('change' => 'js: function() {
                                                     $.ajax({
                                                        ...
                                                        type : "POST",
                                                        data :  { product_id:  $(this).val(), departement_id: $("#Out_departement_id").val()},
                                                        success : function(data){
                                                            obj = JSON.parse(data);
...
//                                    'oninput'=>'js:calculate()'
                    ...
                    if ($model->isNewRecord == FALSE) {
                        foreach ($mOutDet as $o) {
                            echo '<tr>
                        <td>
                            <input type="hidden" name="OutDet[product_id][]" value="' . $o->product_id . '"/>
                            <input type="hidden" name="OutDet[price][]" value="' . $o->price . '"/>
                            <input type="hidden" name="OutDet[qty][]" value="' . $o->qty . '"/>
                            <input type="hidden" name="OutDet[total][]" class="detTotal" value="' . $o->price * $o->qty . '"/>
