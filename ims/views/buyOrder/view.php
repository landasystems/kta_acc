<?php
$this->setPageTitle('Lihat Buy Orders | ID : ' . $model->id);
...
$this->widget('bootstrap.widgets.TbMenu', array(
    'type' => 'pills',
    'items' => array(
        array('visible' => landa()->checkAccess('BuyOrder', 'c'),'label' => 'Tambah', 'icon' => 'icon-plus', 'url' => Yii::app()->controller->createUrl('create'), 'linkOptions' => array()),
        array('label' => 'Daftar', 'icon' => 'icon-th-list', 'url' => Yii::app()->controller->createUrl('index'), 'linkOptions' => array()),
        array('visible' => landa()->checkAccess('BuyOrder', 'u'),'label' => 'Edit', 'icon' => 'icon-edit', 'url' => Yii::app()->controller->createUrl('update', array('id' => $model->id)), 'linkOptions' => array()),
            ...
            <div class="print">
                ...
                <div id="myModal" style="width: 740px" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                    <div class="modal-header">
                        ...
                        <h3 id="myModalLabel">Invoice BUY ORDER</h3>
                    </div>
                    <div class="modal-body">
                        <div class="printableArea">
                            <?php
                            $details = $mBuyOrderDet;
                            $siteConfig = SiteConfig::model()->listSiteConfig();
                            $city = (isset($model->ProductSupplier->city_id)) ? $model->ProductSupplier->city_id :'';
                            $city = City::model()->findByPk($city);
                            $cityName = (isset($city->name)) ? $city->name : '-';
                            $provinceName = (isset($city->Province->name)) ? $city->Province->name : '-';
                            $suppliername = (isset($model->ProductSupplier->name)) ? $model->ProductSupplier->name :'-';
                            $addressname = (isset($model->ProductSupplier->address)) ? $model->ProductSupplier->address :'-';
                            $phone = (isset($model->ProductSupplier->phone)) ? $model->ProductSupplier->phone :'-';
                            ...
                            $content = $siteConfig->report_buy_order;
                            $content = str_replace('{invoice}', $model->code, $content);
                            $content = str_replace('{name}', $suppliername, $content);
                            $content = str_replace('{city}', $cityName, $content);
                            $content = str_replace('{province}', $provinceName, $content);
                            $content = str_replace('{address}', $addressname, $content);
                ...
                <tr>
                    <td style="vertical-align: top !important" class="span6">
                        ...
                        <div class="row-fluid">
                            ...
                            <div class="span8" style="text-align:left">
                                <?php
                                echo CHtml::dropDownList('BuyOrder[supplier_id]', $model->supplier_id, CHtml::listData(User::model()->findAll(array()), 'id', 'name'), array(
                    ...
                    <td style="vertical-align: top !important" class="span6">
                        ...
                        echo $form->textFieldRow(
                                $model, 'date_delivery', array('class' => 'span2', 'prepend' => '<i class="icon-calendar"></i>', 'disabled' => 'true')
                        );
                        ?>  
                    </td>
                </tr>
            ...
            <table class="responsive table table-bordered">
                ...
                <tbody>                                      
                    ...
                    <tr>
                        ...
                        <td>
                            <?php echo landa()->rp($model->payment); ?>
                        </td>
                    </tr>
                </tbody>
            </table>
