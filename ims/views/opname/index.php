...
$('.search-form form').submit(function(){
    $.fn.yiiGridView.update('opname-grid', {
        data: $(this).serialize()
    });
    return false;
});
...
<?php
$buton="";
if(landa()->checkAccess('StockOpname', 'r')){
   $buton .= '{view}'; 
}
if(landa()->checkAccess('StockOpname', 'd')){
   $buton .= '{delete}'; 
}
if(landa()->checkAccess('StockOpname', 'u')){
   $buton .= '{update}'; 
}
$this->widget('bootstrap.widgets.TbGridView', array(
    ...
    'columns' => array(
        ...
        array('header' => 'Status',
            'name' => 'is_processed',
            'type' => 'raw',
            'value' => '($data->is_processed==0) ? "<span class=\"label label-warning\">Open</span>" : "<span class=\"label label-info\">Close</span>"',
        ),
        array(
